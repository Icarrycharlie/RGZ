...
async function loadCurrentRate() {
  ...
  document.getElementById('result').textContent = `Курс RON сегодня: ${rate.toFixed(2)} ₽`;
  setupConverter(rate);
}
...
function setupConverter(rate) {
  ...
  rubInput.addEventListener('input', () => {
    if (rubInput.value)
      ronInput.value = (parseFloat(rubInput.value) / rate).toFixed(2);
    else ronInput.value = '';
  });

  ronInput.addEventListener('input', () => {
    if (ronInput.value)
      rubInput.value = (parseFloat(ronInput.value) * rate).toFixed(2);
    else rubInput.value = '';
  });
}
